...
def check_bank():
    ...
    person_age = st.number_input('Возраст', min_value=0.0, max_value=100.0,
                                 value=20.0, step=1.0)
    ...
    loan_famnt = st.number_input('Сумма кредита', min_value=0.0, step=500.0)
    loan_intent = st.selectbox('Цель кредита',
                               ['EDUCATION', 'HOMEIMPROVEMENT', 'MEDICAL', 'PERSONAL', 'VENTURE', 'DEBTCONSOLIDATION'])
    loan_int_rate = st.number_input('Процент стажа', min_value=0.0)
    loan_percent_income = st.number_input('Доход и кредит', min_value=0.0, max_value=1.0)
    cb_person_cred_hist_length = st.number_input('Длинна кредитной истории', min_value=0.0, step=0.1)
